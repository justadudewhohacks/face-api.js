        ...
        return tslib_1.__generator(this, function (_c) {
            switch (_c.label) {
                ...
                case 2:
                    ...
                    filteredRegions = indices.map(function (idx) { return new MtcnnBox_1.MtcnnBox(onetOuts[idx].regions.arraySync()[0][0], onetOuts[idx].regions.arraySync()[0][1], onetOuts[idx].regions.arraySync()[0][2], onetOuts[idx].regions.arraySync()[0][3]); });
                    ...
                    if (filteredBoxes.length > 0) {
                        ...
                        points = indicesNms.map(function (idx, i) {
                            return Array(5).fill(0).map(function (_, ptIdx) {
                                return new tfjs_image_recognition_base_1.Point(((onetOuts[idx].points.arraySync()[0][ptIdx] * (finalBoxes[i].width + 1)) + finalBoxes[i].left), ((onetOuts[idx].points.arraySync()[0][ptIdx + 5] * (finalBoxes[i].height + 1)) + finalBoxes[i].top));
                            });
                        });
                    }
