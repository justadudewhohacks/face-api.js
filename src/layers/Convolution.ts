...
export class Convolution extends Layer {
  ...
  constructor(name: string, stride: [number, number], channelsIn: number, channelsOut: number, kernelSize: number, batchNormOptionals: BatchNormOptionals | null = null) {
    super(name)
    this._stride = stride
  ...
  protected _getParamShapes(): number[][] {
    return [this.filterShape, ...this._biasOrBn.getParamShapes()]
  }
